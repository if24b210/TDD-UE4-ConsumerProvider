...
const cemeteriesUrlOnline = 'https://data.wien.gv.at/daten/geo?service=WFS&request=GetFeature&version=1.1.0&typeName=ogdwien:FRIEDHOFOGD&srsName=EPSG:4326&outputFormat=json';
...
async function loadCemeteries() {
    ...
    try {
        ...
        loadInfo.innerText = 'Cemeteries: erfolgreich online geladen';
        window.cemeteriesData = data;

        renderCemeteries(data);
    } catch (error) {
        console.warn('Cemeteries: Fehler beim Laden der Online-Daten, versuche lokale Datei...', error);
        ...
        try {
            ...
            loadInfo.innerText = 'Cemeteries: lokal geladen (Fallback)';
            window.cemeteriesData = data;

            renderCemeteries(data);
        } catch (fallbackError) {
            console.error('Cemeteries: Fehler beim Laden der lokalen Datei', fallbackError);
            loadInfo.innerText = 'Cemeteries: Fehler beim Laden der Daten';
        }
    }
}
