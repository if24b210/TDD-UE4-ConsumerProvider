...
window.playgroundLayerGroup = window.playgroundLayerGroup || L.layerGroup().addTo(window.map);
...
function renderPlaygrounds() {
    ...
    const filteredFeatures = window.playgroundData.features.filter(f => {
        ...
        if (cbToddler.checked && typeDetail === 'Kleinkinderspielplatz') return true;
        if (cbGenerational.checked && typeDetail === 'Generationenspielplatz') return true;
        if (cbBall.checked && typeDetail === 'Ballspielplatz') return true;
        if (cbBallCage.checked && typeDetail === 'Ballspielkäfig') return true;
        if (cbForrest.checked && typeDetail === 'Waldspielplatz') return true;
    ...
    const fc = {    // FeatureCollection für die gefilterten Spielplätze
